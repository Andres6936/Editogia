			...
			else
			{
				...
				{ // Not typing.
					...
					else if (ch == '"')
					{
						...
						{
						case LINE_XXXX:
							pen.symbol = LINE_XOXO;
							break;
						case LINE_XOXO:
							pen.symbol = LINE_OXOX;
							break;
						case LINE_OXOX:
							pen.symbol = LINE_XXOO;
							break;
						case LINE_XXOO:
							pen.symbol = LINE_OXXO;
							break;
						case LINE_OXXO:
							pen.symbol = LINE_OOXX;
							break;
						case LINE_OOXX:
							pen.symbol = LINE_XOOX;
							break;
						case LINE_XOOX:
							pen.symbol = LINE_XXOX;
							break;
						case LINE_XXOX:
							pen.symbol = LINE_XXXO;
							break;
						case LINE_XXXO:
							pen.symbol = LINE_OXXX;
							break;
						case LINE_OXXX:
							pen.symbol = LINE_XOXX;
							break;
						case LINE_XOXX:
...
*/
	...
	do
	{
		...
		{ // else if (cur && cur->name == "e_elelist")
			...
			{
				if (selected && selected->type() == ELE_NUMBER)
				{ // Editing a number
					...
					else if (ch == KEY_BACKSPACE || ch == 127 || ch == 8)
					{
						if (!alltext.empty())
						{
							...
							if (cur->sizex >= selected->sizex)
								...
								i_ele.set_data("e_list_values",
										break_into_lines(alltext, cur->sizex));
...
void draw_box(interface& edited, int x1, int y1, int x2, int y2)
{
	...
	{ // Not a line-drawing box; so draw & fill
		for (int x = x1; x <= x2; x++)
		{
			for (int y = y1; y <= y2; y++)
				paint(edited, x, y);
