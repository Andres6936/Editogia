...
#include "Editogia/Math/rng.h"    // For Race_datum::get_city_name()
 ...
 */
	...
	for (int i = 0; i < name.size(); i++)
	{
		...
		{
			if (i == 0)
			{ // Start of the name; just remove the *
				name = name.substr(1);
				i--;
			}
			else if (i == 1)
			{  // Again, just remove the *
...
// Once again, just remove the *
		...
		else if (name[i] == '?')
		{
			...
			{  // Remove the ? AND the character before it.
				std::stringstream new_name;
		...
		else if (name[i] == '!')
		{
			...
			{  // Remove the ! AND the character after it.
				std::stringstream new_name;
...
std::string race_skill_name(Race_skill skill)
{
	...
	case SKILL_TRADE:
		return "trade";
	case SKILL_ESPIONAGE:
		return "espionage";
	case SKILL_MAGIC:
		return "magic";
	case SKILL_EARTH_MAGIC:
		return "earth magic";
	case SKILL_WATER_MAGIC:
		return "water magic";
	case SKILL_AIR_MAGIC:
		return "air magic";
	case SKILL_FIRE_MAGIC:
		return "fire magic";
	case SKILL_LIFE_MAGIC:
		return "life magic";
	case SKILL_DEATH_MAGIC:
		return "death magic";
	case SKILL_MAX:
		return "BUG - SKILL_MAX";
	default:
		return "BUG - Unnamed Race_skill";
	}
