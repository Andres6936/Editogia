...
#include "Editogia/Gea/world.h"  // Needed for randomize_properties()
...
// Passing true to get_best_crop() prioritizes food output.
		...
		if (best_index == -1 && best_res_index == -1)
		{ // No useful tiles!
			done = true;
		}
		else
...
// Insert it in the proper place.
			...
			if (!inserted)
			{ // Didn't find a place to put it; push it to the end.
...
// We can build it, and we want to build it, so build it!
			if (add_building(type))
			{ // add_building() returns false on fail
				return true;
			}
		}
