 ...
 */
			...
			if (change)
			{ // Pick a new luxury to demand!  Probably!
...
// If we don't meet demand, give partial morale.
		...
		{  // city didn't have enough resources!
...
std::string citizen_type_name(Citizen_type type, bool plural)
{
	...
	case CIT_PEASANT:
		return (plural ? "peasants" : "peasant");
	case CIT_MERCHANT:
		return (plural ? "merchants" : "merchant");
	case CIT_BURGHER:
		return (plural ? "burghers" : "burgher");
	case CIT_MAX:
		return "BUG - CIT_MAX";
	default:
...
std::string morale_mod_type_name(Morale_mod_type type)
{
	switch (type)
	{
	case MORALE_MOD_NULL:
		return "NULL";
	case MORALE_MOD_LUXURY:
		return "luxury";
	case MORALE_MOD_BUILDING:
		return "building";
	case MORALE_MOD_FESTIVAL:
		return "festival";
	case MORALE_MOD_HUNGER:
		return "hunger";
	case MORALE_MOD_DEATH:
		return "death";
	case MORALE_MOD_MAX:
		return "BUG - MORALE_MOD_MAX";
	default:
		return "BUG - Unnamed Morale_mod_type";
	}
...
std::string cause_of_death_text(Cause_of_death cause, bool plural)
{
	...
	case DEATH_MAX:
		return "BUG - DEATH_MAX";
	default:
		return "Unnamed Cause_of_death";
	}
