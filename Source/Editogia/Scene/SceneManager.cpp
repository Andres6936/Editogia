...
// Menu setup!  Put menus here.
	...
	addMenu(TypeMenu::Buildings,
			"Status",
			"Build"
	);

	addMenu(TypeMenu::World,
			"View Map"
	);

	addMenu(TypeMenu::Help,
			"Index",
			"Search"
	);
...
<c=pink>G<c=ltred> to generate one.<c=/>");
	...
	long ch;
	while (true)
	{  // Loop until we exit the function via L N or Q
		ch = input();
...
// to print, we need to put in black spaces so the mineral starts at the right.
				for (int n = 0; n < 20; n++)
				{  // 15 for the name, 5 for the amount
					ss_data << " ";
				}
			}
...
// So we set them inside the loop.
		...
		if (fields_empty == 0)
		{  // We don't care since all fields are used
			i_food.set_data("num_free_peasants", c_dkgray);
		}
		else
		{  // We care since we could utilize a field
			if (free_peasants == 0)
			{
		...
		if (days_left == -1)
		{ // We'll never run out of food at this rate!
...
// peasants to work at this farm... so make sure we can!
			...
			if (did_it)
			{ // We actually changed # of crops!  Update interface.
				list_farm_crops(cur_farm, i_food);
...
// Display hunting records
	...
	if (pl_city->hunt_record_days <= 0)
	{ // It's -1 until we build a camp
...
// Minerals used shouldn't change during this function, so we can set it here.
	...
	while (!done)
	{
		...
		if (free_shafts == 0)
		{  // We don't care since all shafts are used
			i_mining.set_data("num_free_peasants", c_dkgray);
		}
		else
		{  // We care since we could utilize a shaft
			if (free_peasants == 0)
			{
...
// peasants to work at this mine... so make sure we can!
			...
			if (did_it)
			{ // We actually changed a # of minerals!  Update interface.
				list_mine_minerals(cur_mine, mineral_indices, i_mining);
// Reset our position in the list to what it was previously!
...
// Go through all the buildings and fill our string vectors with their info
	for (int i = 0; i < buildings.size(); i++)
	{
		...
		else if (bldg->pos.x != -1)
		{ // It belongs to an area (and isn't closed)
 ...
 */
			...
			else if (cur_bldg->builds_resource())
			{ // Any manufacturing
...
// Check move_index to see if we need to set the selected item
			...
			else if (cur_bldg->get_housing() > 0)
			{ // Any housing
...
// Add an item to the production queue.
		...
		case 'A':
			if (cur_bldg && adjusting_production)
			{
				...
				if (pick_recipe(cur_bldg, new_recipe))
...
// Did we pick an option from our list?
		if (ch >= '1' && ch <= '9')
		{
			...
			{ // Actually building a building, not picking a category
...
// If we're at the top level, or in the building queue, we leave the screen.
			...
			{ // TAB
				...
				if (editing_queue)
				{ // We moved to control the queue list
...
// Warn the player if this will waste effort
						...
						if (do_it)
						{
							...
							if (!pl_city->cancel_queued_building(index))
							{ // BAD ERROR
...
// Attempt to keep the cursor where it was
			...
			default:
				if (editing_queue)
				{ // Keybindings only work when on queue list
...
// Now, fill list_options with all the names of the different Building_categorys
		for (int i = 1; i < BUILDCAT_MAX; i++)
		{ // Start at 1 to skip BUILDCAT_NULL
...
{
	if (editing_queue)
	{  // Help refers to editing the building queue
...
");
	...
	else if (build_cat == BUILDCAT_NULL)
	{ // Help refers to picking a category
	...
	{  // Help refers to picking a building
...
// Populate our vectors (and the interface's lists)
				...
				for (int i = 0; i < results.size(); i++)
				{
					...
					{  // Not a redirect; use the normal type name.
			...
			{  // We were selecting an article from the list.
...
std::string data_mode_name(Data_mode mode)
{
	...
	case DATA_MODE_MESSAGES:
		return "Messages";
	case DATA_MODE_MAX:
		return "BUG - DATA_MODE_MAX";
	default:
		return "Unnamed Data_mode";
	}
