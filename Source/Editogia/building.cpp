...
{
	if (!open)
	{ // Closed buildings don't produce anything!
...
// First, look at the production vector of our type.
		for (int i = 0; i < datum->production.size(); i++)
		{
			...
			{ // Gain non-meta resources
				resource_gains[res_amt.type] += res_amt.amount * workers;

			}
			else
			{  // Meta resources are more complicated!
				int skill = 0;
				switch (res_amt.type)
				{
				case RES_FARMING:
...
// We've exhausted the tile's supply of wood!
					...
					{  // We have NOT exhausted the supply of wood.
...
// Check if we can build the recipe today.
			int num_built = 0;
			if (recipe->days_per_unit <= 1)
			{ // We can build it every day!
				num_built = workers;
				if (recipe->units_per_day > 0)
				{  // We can build multiples every day!
					num_built *= recipe->units_per_day;
				}
			}
			else
			{  // It takes multiple days to build a unit.
...
// and will fire the workers.
			...
			if (!tile_min)
			{  // The mineral doesn't exist on the map tile!
				exhausted = true;
...
// We exhausted the supply of the mineral!
				...
				{  // Mineral is neither infinite nor exhausted
					mineral_gains[min_mined.type] += min_mined.amount;
					if (real)
					{
				...
				if (real && exhausted)
				{  // We used up the map's supply of min_mined!
...
std::map<Resource, int> Building::get_maintenance()
{
	...
	if (!bd_data)
	{  // Should never ever happen, but why not
		return ret;
	}
...
const std::string Building_datum::toJSON() const noexcept
{
	...
	for(const auto& cost : build_costs)
	{
		...
		buildCostsObject.add(costObject);
	}

	doc["cost"] = buildCostsObject;
...
std::string building_category_name(Building_category category)
{
	...
	case BUILDCAT_NULL:
		return "NULL";
	case BUILDCAT_MANUFACTURING:
		return "manufacturing";
	case BUILDCAT_MAX:
		return "BUG - BUILDCAT_MAX";
	default:
		return "Unnamed Building_category";
	}
