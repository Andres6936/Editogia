...
// On a tie, choose whichever has lower difficulty.
	...
	if (best_index == -1)
	{ // No food-producing animals here!
		return ANIMAL_NULL;
	}
...
{
	Terrain_datum* ter_dat = get_terrain_datum();
	if (ter_dat->wood_cleared_type == TER_NULL)
	{ // Doesn't change.
		return;
	}
...
//       For now, it's just northwest => southeast
		...
		case DIRFULL_NORTH:
			...
			y = 0;
			break;
		case DIRFULL_SOUTHWEST:
			x = 0;
			y = end;
			break;
		case DIRFULL_NORTHEAST:
			x = end;
			y = 0;
			break;
...
// Store the center tile in a buffer just in case the lake overwrites it
		...
		{ // Not a lake BOOOOO GIMME A LAKE
